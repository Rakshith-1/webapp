...
resource "null_resource" "install_helm_release1" {
  provisioner "local-exec" {
    command = "sudo -u root /root/bin/helm upgrade my-release-fe s3://frontendchartrakshit/frontend-foldername/frontend-0.1.0.tgz"
  }
...
resource "null_resource" "install_helm_release2" {
  provisioner "local-exec" {
    command = "sudo -u root /root/bin/helm upgrade my-release-be s3://backendchartrakshit/backend-foldername/backend-0.1.0.tgz"
  }
